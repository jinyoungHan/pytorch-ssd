...
class VGG16(nn.Module):
    """
    VGG-16 convolutions to produce lower-level feature maps.
    """
    def __init__(self):
    ...
    def load_pretrained_layers(self):
        ...
        """
        # Current state of base: conv1 - conv7
        ...
        # fc6
        fc6_weight = pretrained_state_dict['classifier.0.weight'].view(4096, 512, 7, 7)  # (4096, 512, 7, 7)
        fc6_bias = pretrained_state_dict['classifier.0.bias']  # (4096)
        state_dict['conv6.weight'] = decimate(fc6_weight, m=[4, None, 3, 3])  # (1024, 512, 3, 3)
        state_dict['conv6.bias'] = decimate(fc6_bias, m=[4])  # (1024)

        # fc7
        fc7_weight = pretrained_state_dict['classifier.3.weight'].view(4096, 4096, 1, 1)  # (4096, 4096, 1, 1)
        fc7_bias = pretrained_state_dict['classifier.3.bias']  # (4096)
        state_dict['conv7.weight'] = decimate(fc7_weight, m=[4, 4, None, None])  # (1024, 1024, 1, 1)
        state_dict['conv7.bias'] = decimate(fc7_bias, m=[4])  # (1024)
...
class Auxiliary(nn.Module):
    """
    Extra Feature Layers to produce higher-level feature maps.
    """
...
class SSD300(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        self.base = VGG16()
        self.aux_convs = Auxiliary()
        self.pred_convs = Prediction(n_classes)
    ...
    def forward(self, x):
        ...
        conv4_3_feats, conv7_feats = self.base(x)  # (N, 512, 38, 38), (N, 1024, 19, 19)
    ...
    def create_prior_boxes(self):
        ...
        fmaps = list(fea_map_dim.keys())

        prior_boxes = []

        for k, fmap in enumerate(fmaps):
            for i in range(fea_map_dim[fmap]):
                for j in range(fea_map_dim[fmap]):
                    cx = (j + 0.5) / fea_map_dim[fmap]
                    cy = (i + 0.5) / fea_map_dim[fmap]
