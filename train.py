...
from model import SSD300, MultiBox
...
epochs = 25  # number of epochs to run without early-stopping
...
def main():
    ...
    criterion = MultiBox(priors_cxcy=model.priors_cxcy).to(device)

    # Custom dataloaders
    train_dataset = VOC_Dataset(data_folder,
                                     split='train',
                                     keep_difficult=keep_difficult)
    val_dataset = VOC_Dataset(data_folder,
                                   split='test',
                                   keep_difficult=keep_difficult)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        # Batches
        for i, (images, boxes, labels, difficulty) in enumerate(val_loader):
